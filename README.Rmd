...
calibration <- calibrate(country = "Afghanistan", deaths = 25, reporting_fraction = 1,
                         ...
                         replicates = 100, dt = 0.25)

```

Simulation replicates are aligned to the current death total and the outputs are 
returned as a `squire_simulation` object. -insert text here about how to plot-

```{r deaths over time}

x <- format_output(calibration, var_select = c("n_E2_I")) %>%
  mutate(replicate = factor(replicate)) %>%
  filter(t < 14)
ggplot(x, aes(x = t, y = y, col = replicate)) +
  geom_line() +
