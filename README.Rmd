...
These could be created by using the `format_output` on the output and selecting relevant 
compartments but this is quite slow to do, so we provide some helper functions. These include:
...
The model can be simply calibrated to the cumulative time series of deaths reported in settings. 
This can be done using the `calibrate` function. For example, let's generate some dummy time series
data and calibrate to it:
...
returned as a `squire_simulation` object.

These can be plotted by either extracting the desired variables again using 
`format_output` and providing the date that the deaths relates to. E.g. for the 
following 4 weeks:
...
Alternatively, there are a few unexposed functions for plotting these outputs:
