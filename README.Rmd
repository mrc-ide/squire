...
If the `country` is provided, the `population` and `baseline_contact_matrix` 
...
```{r death ICU and bed summaries}

# Still to do

```

The number of deaths over time will depend heavily on the available healthcare
capacity of a country, with excess mortality occurring when critical care patients, 
who require mechanical ventilation, are unable to access one due to shortages:

```{r death ICU and bed summaries with higher ICU beds}
...
Simulation replicates are aligned to the current death total and the outputs of
each simulation replicate are returned in long format. To plot the deaths over
time up until the next week:
