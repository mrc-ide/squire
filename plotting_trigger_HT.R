...
LIC_hosp_dem <- LIC_hosps_1 + LIC_hosps_2 + LIC_hosps_3 + LIC_hosps_4 + LIC_hosps_5 + LIC_hosps_6 + LIC_hosps_7 + LIC_hosps_8 + LIC_hosps_9 + LIC_hosps_10
...
trigger$scenario <- "Scenario 5"

LIC_lockdown <- LIC$time_in_lockdown[1:(time_period/dt),4]
...
ggplot(trigger) +
  ...
  geom_ribbon( data = subset(trigger, compartment== "ICU bed demand"),
               aes(x=time, ymin = 0, ymax = LIC_lockdown * 40), alpha = 0.1)

p2/
  p2 + plot_layout(guides = 'collect')

ggsave("C:/Users/ht1212/Imperial College London/ncov - Documents/2019-nCoV/LMIC/Senegal/fig_3_A.pdf", width=12, height=6, dpi=600)
...
ggplot(deaths_age) +
  geom_col(mapping=aes(x=age_groups, y=prop_deaths), fill="#FDBF6F", show.legend = FALSE) +
...
ggsave("C:/Users/ht1212/Imperial College London/ncov - Documents/2019-nCoV/LMIC/Senegal/deaths_scenario_5.pdf", width=4, height=4, dpi=600)

ggsave("C:/Users/ht1212/Imperial College London/ncov - Documents/2019-nCoV/LMIC/Senegal/deaths_scenario_5_GOOD_outs.pdf", width=4, height=4, dpi=600)

toot <- lapply(seq_along(1:dim(LIC$model_output)[3]), function(p) {
  temp <- rollapply(LIC$model_output[, index$IOxGetLive1, 1], 1/dt, sum, partial = TRUE, align = "right") #take time points to days
  daily_deaths_new <- temp[seq(1, length(temp[,11]), 10), ]
  daily_deaths_new <-  apply(daily_deaths_new, 2, sum) #to all the row - sum the age groups for that replicate
  # 1/dt so that is the 10th elements = daily
  # list of 10 elements over each p value
...
ggsave("C:/Users/ht1212/Imperial College London/ncov - Documents/2019-nCoV/LMIC/Senegal/figures/age_cases_triggered.pdf", width=4, height=4, dpi=600)
